...
Wantedly・RSS・HackerNewsから「AI導入/業務効率化」に課題を持つ企業を自動検出
...
def save_cost(input_tokens, output_tokens, label, model="claude"):
    ...
    log["monthly"][month]["calls"].append({
        "date": TODAY, "label": label, "model": model,
...
def call_claude(prompt, max_tokens=1500, label="sales_api"):
    """Claude APIを呼び出す（Qwen3失敗時のフォールバック用）"""
    ...
    for feed_url in WANTEDLY_RSS_FEEDS:
        try:
            ...
            for entry in entries[:10]:
                ...
                if title and link:
                    items.append({"title": title, "url": link, "text": desc, "source": "wantedly"})
            time.sleep(1)
        except Exception as e:
        ...
        try:
            ...
            for entry in entries[:8]:
                ...
                if desc_m:
                    ...
                    desc = re.sub(r"<[^>]+>", "", desc).strip()[:300]
                if title and link:
                    items.append({"title": title, "url": link, "text": desc, "source": feed["source"]})
            time.sleep(1)
        except Exception as e:
...
def analyze_targets_batch(targets):
    ...
    prompt = (
        "あなたはAI自動化ツール「Brain」の営業担当です。\n"
        "以下の記事・投稿を見て、Brainの営業ターゲットになりうる企業・個人を分析してください。\n\n"
        "【Brainとは】\n"
        "毎日自動でAI情報収集・要約・X投稿文生成・営業リスト作成を行う自律型エージェント。\n"
        "メニュー: スポット受託¥49,800〜・月次サポート¥30,000/月\n\n"
        "【収集した記事一覧】\n" + targets_text + "\n\n"
        "各記事について以下をJSON配列で返してください。上位10件のみ選んでください。\n\n"
        "[\n"
        "  {\n"
        "    \"rank\": 優先順位,\n"
        "    \"name\": \"企業名または発信者名\",\n"
        ...
        "    \"why_brain\": \"なぜBrainが必要か（1〜2文）\",\n"
        "    \"push_point\": \"営業で押せるポイント（1文）\",\n"
        "    \"first_message\": \"最初のDMの書き出し（40字以内）\",\n"
        "    \"plan\": \"おすすめプラン名\",\n"
        "    \"price\": \"想定単価\",\n"
        "    \"probability\": \"高/中/低\"\n"
        "  }\n"
    ...
    response = call_qwen(prompt, max_tokens=2000, label="sales_analyze_batch")
    try:
...
def save_sales_list(analyzed_targets, total_scanned):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("> スキャン件数: " + str(total_scanned) + "件 → 営業候補: " + str(len(analyzed_targets)) + "件\n\n---\n\n")
...
def main():
    ...
    print("  モード: 分析=Qwen3（無料）/ フォールバック=Claude")
